...
public class AssignableDivideBy extends AssignableStatement {
    ...
    public AssignableDivideBy(PersonalityMatrix matrix, int fieldindex, int palletIndex1, int palletIndex2) {
        super("DIVIDE", null);
        this.matrix = matrix;
        ...
        if (fieldindex >= 0 && matrix.getCode().length > fieldindex)
            ...
            if (matrix.getCode()[fieldindex] instanceof AssignableField && matrix.getCode()[palletIndex1] instanceof AssignableField&& matrix.getCode()[palletIndex2] instanceof AssignableField) {
                if (((AssignableField) matrix.getCode()[fieldindex]).isReadOnly())
                    return;
                if (matrix.getIntAtField(palletIndex2) == 0 )
                    return;
    ...
    public String toString() {
        return "DIVIDE (" + fieldindex + "," + palletIndex1 +  "," + palletIndex2 +");";
    }

    @Override
    public AssignableCode clone(PersonalityMatrix matrix) {
        return new AssignableDivideBy(matrix, fieldindex, palletIndex1, palletIndex2);
    }
